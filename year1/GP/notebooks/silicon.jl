...
# ╔═╡ 660e52b0-fe44-11ee-1862-375123ba1ddb
begin
	using DFTK
	using Plots
	using Unitful
...
# ╔═╡ fc3be564-edeb-44e4-a564-b3449f5fb1c9
begin
	a_const = 10.26 # Silicon lattice constant in Bohr
	s = 0.05
	a_list = (a_const - 0.06*a_const):s:(a_const + 0.06*a_const)
end

# ╔═╡ a30856e3-89c7-420e-b2e3-611ee08891ae
begin
	total_energy_list = []
	for a in a_list
		lattice = a / 2 * [[0 1 1.];
		...
		push!(total_energy_list, scfres.energies.total)
		# println(scfres.energies.total)
		
	end
end

# ╔═╡ 0b84b89d-c648-41eb-9197-7775358e0e51
scatter(a_list, total_energy_list; xlabel="Lattice constant", ylabel="Energy", legend=false)

# ╔═╡ 4b87d3e7-3222-41cc-882d-625d31155537
total_energy_list

# ╔═╡ 1face14d-f211-4fb5-9371-912b9f8b2889
f = GP(SqExponentialKernel())

# ╔═╡ 863814c3-8036-48e6-a98a-a3bd52595ebb
md"""
step size = $(@bind step PlutoUI.Slider(1:1:5; default=2, show_value=true))

noise degree = $(@bind noise_degree PlutoUI.Slider(-10:1:0; default=-8, show_value=true))

"""

# ╔═╡ 5c4319b8-f95b-4f83-8284-2d3447f1a6fa
begin
	# step = 4
	n = length(a_list)
	partition = floor(Int, 0.34 * n)
	
	x_train = AbstractArray{Float64}([a_list[1:step:partition]; 
									 a_list[(2*partition + 1):step:end]])
	y_train = AbstractArray{Float64}([total_energy_list[1:step:partition];
									 total_energy_list[(2*partition + 1):step:end]])
	
	x_test = AbstractArray{Float64}(a_list[(partition + 1):step:2*partition])
	y_test = AbstractArray{Float64}(total_energy_list[(partition + 1):step:2*partition])
end

# ╔═╡ 847a9738-06e5-4c4e-85ba-c8c1d3a18bb1
begin
	noise_var = 10.0^(noise_degree)
		
	# Finite projection of `f` at inputs `x_train`.
	# Added Gaussian noise with variance `noise_var`.
	fx = f(x_train, noise_var)

	p_fx = posterior(fx, y_train)
end

# ╔═╡ 1525a642-62a2-40e9-8410-e5a8f74abcb5
-logpdf(fx, x_train)

# ╔═╡ 33b656a9-4a96-44bf-b044-1b6dfca31c98
-logpdf(p_fx(x_test, noise_var), y_test)

# ╔═╡ ba89887d-3422-49c8-927f-36ddf748accf
begin
	scatter(
	    x_train,
	    y_train;
	...
	plot!(9:0.01:12, p_fx; label="SqExponentialKernel", ribbon_scale=2)
end

# ╔═╡ 9e7be1e3-570a-4c8e-a749-9b3bf7bae1a8
# begin
# 	test_neg_logpdf = []
# 	push!(test_neg_logpdf, -logpdf(p_fx(x_test, noise_var), y_test));
# end

# ╔═╡ 2b8d033e-1acc-4e24-bcd4-476aaf278623
begin
	k_1 = PolynomialKernel(; c=2.0, degree=2) #∘ LinearTransform(randn(4, 1))
	f_1 = GP(k_1) 
	fx_1 = f_1(x_train, noise_var)
	
	# compute the log-likelihood of the outputs of the training dataset
	println("The negative log-likelihood of the train outputs: ", -logpdf(fx_1, y_train))
	
	# compute the posterior Gaussian process given the training data
	p_fx_1 = posterior(fx_1, y_train)
	println("The negative log-likelihood of the test outputs: ", -logpdf(p_fx_1(x_test, noise_var), y_test))

	# push!(test_neg_logpdf, -logpdf(p_fx_1(x_test, noise_var), y_test));
end

# ╔═╡ ddf3b112-2dbb-4ea9-8c72-854e9f2f453a
begin
	scatter(
	    x_train,
	    y_train;
	...
	scatter!(x_test, y_test; label="Test Data")
	plot!(9:0.065:12, p_fx; label="SqExponentialKernel", ribbon_scale=2)
	plot!(9:0.065:12, p_fx_1; label="PolynomialKernel", ribbon_scale=2)
end

# ╔═╡ f0f87cfd-40b2-41e3-b7a4-e2e52fbdf45c
begin
	# g(x) = x * x; t = FunctionTransform(g);

	k_2 = 0.5 * SqExponentialKernel() * LinearKernel(; c=0.5) + 0.4 * k_1
	
	f_2 = GP(k_2) 
	fx_2 = f_2(x_train, noise_var)
		
	# compute the log-likelihood of the outputs of the training dataset
	println("The negative log-likelihood of the train outputs: ", -logpdf(fx_2, y_train))
	
	# compute the posterior Gaussian process given the training data
	p_fx_2 = posterior(fx_2, y_train)
	println("The negative log-likelihood of the test outputs: ", -logpdf(p_fx_2(x_test, noise_var), y_test))

	# push!(test_neg_logpdf, -logpdf(p_fx_2(x_test, noise_var), y_test));
end

# ╔═╡ 59666765-d822-440f-903e-b8633f0a96c9
begin
	scatter(
	    x_train,
	    y_train;
	...
	scatter!(x_test, y_test; label="Test Data")
	plot!(0:0.001:15, p_fx; label="SqExponentialKernel", ribbon_scale=2)
	plot!(0:0.001:15, p_fx_1; label="k_1", ribbon_scale=2)
	plot!(0:0.001:15, p_fx_2; label="k_2", ribbon_scale=2)
end

# ╔═╡ 2d47e0d2-d47c-4b90-ba37-316d9b8b5c23
begin
	kernels = ["k_1", "SE",  "k_2"]
	
	test_neg_logpdf = [ -logpdf(p_fx_1(x_test, noise_var), y_test),
						-logpdf(p_fx(x_test, noise_var), y_test),
						-logpdf(p_fx_2(x_test, noise_var), y_test)]
	
end

# ╔═╡ dd317365-4431-4928-8f2e-e02c96edeb54
scatter(kernels, test_neg_logpdf, xlabel="Kernels", ylabel="Negative log-likelihood on test examples")
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "e48a8f0224ecfb9301f9b637fc036ad49b5c743f"
...
git-tree-sha1 = "010e6b5e06a9a6b1be6b146a7f29d5848490cc64"
...
git-tree-sha1 = "8f41a90a2ec4c2e6883cce0d06bb9b7c2643c1ce"
...
git-tree-sha1 = "cfcd286261f2593842564b2f6e383a92234621b2"
...
[[deps.spglib_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl"]
git-tree-sha1 = "6b96ad622704c077ca317911935b070523d54299"
