...
# ╔═╡ 6d7c1cf1-ead7-4e33-8516-3f3c1837f9d8
using Optim

# ╔═╡ e0dd4ff3-3e77-43fc-964e-dbf771250d0d
md"""
### Intro to AbstractGPs: one-dimesional regression
"""

# ╔═╡ c33e2844-657e-4e9d-b1fa-b2ea96af3307
begin
	x = [
	    0.8658165855998895,
	    0.6661700880180962,
...
# ╔═╡ a25cbd2c-59ea-4369-9c80-b174d6e87269
begin
	x_train = x[1:8]
	y_train = y[1:8]
	x_test = x[9:end]
...
# ╔═╡ d8449727-846f-43bc-b327-320934bec099
plot(-20:0.1:20, softplus)

# ╔═╡ 64062bf3-8547-46c4-bf35-f6d88a4ef49e
begin
	# The Matern kernel has a fixed variance and length scale  
	# parameters of default value 1.
	kernels = [Matern12Kernel(), Matern32Kernel(), Matern52Kernel()]
end

# ╔═╡ 0f6487c5-e924-4e66-b238-2c6f7cffc991
kernelmatrix(kernels[1], x)

# ╔═╡ ba603da9-a7e3-4e8b-ac3b-ec0f03496c76
begin
	f = GP(kernels[end])
	cov(f(x)) == kernelmatrix(kernels[end], x)
end

# ╔═╡ b4177f65-0ef0-49dc-8f94-0a280f7a4267
mean(f(x)) == zeros(length(x))

# ╔═╡ b09ecc13-e758-4e02-a847-2fc2faa4ac20


# ╔═╡ ceb59322-18a2-41ca-b89d-647755f87ad8
md"""
noise variance = $(@bind noise_var PlutoUI.Slider(0.0:0.02:1.0; default=0.1, show_value=true))
"""

# ╔═╡ 62819234-138b-4e94-a88a-326aad5869cb
begin
	p_fx_list = []
...
# ╔═╡ 87f7b54c-cd0f-405b-b45d-524587406688
begin
	scatter(
	    x_train,
	    y_train;
	...
	plot!(0:0.001:1, p_fx_list[1]; label="Matern12 kernel", ribbon_scale=2)
	plot!(0:0.001:1, p_fx_list[2]; label="Matern32 kernel", ribbon_scale=2)
	plot!(0:0.001:1, p_fx_list[3]; label="Matern52 kernel", ribbon_scale=2)
end

# ╔═╡ 778f4f85-a3db-4171-8477-c14f2e7a3c1c
begin
	fx = f(x_train, noise_var)
	p_fx = posterior(fx, y_train)
...
# ╔═╡ c63d97ea-f274-4d4d-b11e-ff058329f966
(Matern52Kernel()∘ ScaleTransform(10))(2,3)

# ╔═╡ 0ad98651-d751-45a7-8e3b-4e89e0a267da
(Matern52Kernel()∘ ScaleTransform(1))(20,30)

# ╔═╡ e90cdf62-ed48-48bb-8088-5c958a236328
...
# ╔═╡ a37db232-855c-4ae2-9cb3-13ff5fc22d8a
begin
...
# ╔═╡ d480013f-2cda-4aa2-a124-f2af5da2996d
opt.minimizer

# ╔═╡ 2a4935c6-9a9e-4435-80b4-a8646a225566
# The optimized value of the variance (parameter of the Matern kernel)
softplus(opt.minimizer[1])

# ╔═╡ 64a3bd4d-accf-4355-b23c-e34425dc38c4
# The inverse lengthscale (parameter of the Matern kernel)
softplus(opt.minimizer[2])

# ╔═╡ 61c37807-f7e5-4c6e-81fa-ddb421fda0fe
begin
	opt_kernel =
	    softplus(opt.minimizer[1]) *
...
# ╔═╡ f6a2d90e-0f12-4059-a223-74e4988303e2
begin
	scatter(
	    x_train,
	    y_train;
	...
	scatter!(x_test, y_test; label="Test Data")
	plot!(0:0.001:1, opt_p_fx; label="Optimized parameters", ribbon_scale=2)
	plot!(0:0.001:1, p_fx; label="Default parameters", ribbon_scale=2)
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "fe8535598f35c5ecefc7b698419d7535bd6d9d84"
