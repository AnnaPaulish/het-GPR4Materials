...
# ╔═╡ 16933730-f7fc-11ee-2fcc-6dd1ce7fe5fd
begin
	using AbstractGPs
...
# ╔═╡ 98273499-6a57-4ef8-a9a9-ce2efb2d841a
md"""
### Parametric Heteroscedastic Model
"""

# ╔═╡ d8e6acb4-a7f0-4838-9abf-f26aaad1121d
md"""
This example is a small extension of the standard GP regression problem, in which the observation noise variance is a function of the input. 

We work with a simple GP with a Gaussian kernel and heteroscedastic observation variance. Here, the observation noise variance is a **function** of the input. It is assumed to be a simple quadratic form, with a single unknown scaling parameter, in addition to the usual lengthscale and variance parameters of the GP.
"""

# ╔═╡ 28abeae2-77c6-4240-84af-23d37971f509
begin
...
# ╔═╡ 72355f48-cb53-4024-b428-bb87849cdc2e
begin
...
# ╔═╡ dc7d714d-f78f-4a3a-9708-e177a2d91d44
ParameterHandling.value_flatten((
	    s=positive(1.0), l=positive(3.0), σ²=positive(0.1)
	))

# ╔═╡ 4560d830-bbeb-4fc5-ba67-8bf5937c3ba8
positive(1.0)

# ╔═╡ f1f3ed4c-4996-4730-bbc6-11966673b6b8
positive(3.0)

# ╔═╡ 2b3e7f3d-e11c-4681-a769-f9a27ac7110b
flat_θ

# ╔═╡ 6a804887-647c-4c2f-b4dc-431945d68d1c
unflatten(flat_θ)

# ╔═╡ f74c444b-8000-473f-9a72-9897d18a66e6
begin
...
# ╔═╡ 27e80bfe-493d-44c2-87e4-050842e249a3
observation_variance(θ, x::AbstractVector{<:Real})

# ╔═╡ f75c0c02-d423-4a35-80ec-05eded217f41
...
# ╔═╡ add273fc-39ef-4d01-a3a6-cd2b30e9e361
md"""
We use L-BFGS for optimising the objective function. It is a first-order method and hence requires computing the gradient of the objective function. We do not derive and implement the gradient function manually here but instead use reverse-mode automatic differentiation with Zygote. 
"""

# ╔═╡ 6ff99d14-195f-4cc8-aad7-b0a60cd53ed4
...
# ╔═╡ cac62e4a-78f7-4934-8cee-9c8452438771
begin
...
# ╔═╡ 92666633-4a5e-4211-ad64-fcf3806af92d
# The optimal model parameters
θ_final = unflatten(result.minimizer)

# ╔═╡ 64cdf33b-d3cb-47bd-a55d-f51fece8a1f0
begin
	# Compute the posterior GP with these optimal model parameters
	fx_final = build_gpx(θ_final, x)
	f_post = posterior(fx_final, y);
end

# ╔═╡ 5cfc6995-f8b7-4380-b010-425c412e61ee
begin
	...
	with_theme(
	    ...
	    plot(
	        ...
	        bandscale=3,
	        label="posterior (with noise)",
	        color=(:orange, 0.3),
	    )
	    plot!(x, f_post; bandscale=3, label="posterior")
	   # gpsample!(x, f_post; samples=5, color=Set1_4[3])
	    scatter!(x, y; label="y")
	    axislegend()
	    current_figure()
	end
...
# ╔═╡ 0cc6edf3-8bb7-4c88-9028-aebce93323e6
f_post

# ╔═╡ ea6b6572-9084-46cc-8d6e-07aa03730c6a
...
[compat]
AbstractGPs = "~0.5.19"
AbstractGPsMakie = "~0.2.6"
CairoMakie = "~0.10.12"
KernelFunctions = "~0.10.60"
Optim = "~1.7.8"
ParameterHandling = "~0.4.7"
Zygote = "~0.6.68"
"""
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "1249c9d91d99a088a54c7486ec575a6ba63d6d4b"
...
git-tree-sha1 = "72aa3c811dbf8afab877f27727b0007035b48506"
