...
# ╔═╡ 951a86f0-fcc3-11ee-0c09-adb14778d567
begin
	using DFTK
	using Plots
	using Unitful
...
# ╔═╡ 3a4a495a-1e76-4d38-86db-e2b3d0c374f6
begin
	a_const = 10.26 # Silicon lattice constant in Bohr
	a_list = (a_const - 0.06*a_const):0.02:(a_const + 0.06*a_const)
end

# ╔═╡ 4e1e278c-2a02-4dd4-8f40-932435f0b978
begin
	total_energy_list = []
	for a in a_list
		lattice = a / 2 * [[0 1 1.];
...
# ╔═╡ 44d05a2c-eac4-4797-baf4-1bab0a45be7d
length(a_list)

# ╔═╡ d79ad0df-8193-4737-9d05-2b0ca473dbb0
length(total_energy_list)

# ╔═╡ 55ae5d9d-02ee-463e-9450-adeb72fcfac3
scatter(a_list, total_energy_list; xlabel="Lattice constant", ylabel="Energy", legend=false)

# ╔═╡ c82c93f9-a8d5-4fe5-a3d5-5db255823a1d
begin
	n = length(a_list)
	partition = floor(Int, 0.7 * n)
	
	x_train = AbstractArray{Float64}(a_list[1:partition])
	y_train = AbstractArray{Float64}(total_energy_list[1:partition])
	x_test = AbstractArray{Float64}(a_list[(partition + 1):end])
	y_test = AbstractArray{Float64}(total_energy_list[(partition + 1):end])
end

# ╔═╡ cb2fc2e7-9763-4f1f-b108-6cfbf799a102


# ╔═╡ b8f8c3eb-19cb-4de8-a721-0a835e7e6b14
# begin
# 	step = 2
# 	n = length(a_list)
...
# 	x_test = AbstractArray{Float64}(a_list[(partition + 1):2*partition])
# 	y_test = AbstractArray{Float64}(total_energy_list[(partition + 1):2*partition])
# end

# ╔═╡ c953ed60-5863-4e25-9509-55151459e284
f = GP(SqExponentialKernel()) #SqExponentialKernel())

# ╔═╡ db9d22dd-f3e6-487d-a664-fd01f5f18e35
begin
	noise_var = 1.0e-8
...
# ╔═╡ 539e1083-6de1-408e-99ee-b86aa6123268
# The log-likelihood of the train outputs
logpdf(fx, x_train)

# ╔═╡ d843b76b-2d8d-4ec0-8603-c57358d1edf7
# Exact posterior given `y`. This is another GP.
p_fx = posterior(fx, y_train)

# ╔═╡ a5d8b974-99cb-4991-bbb8-63a30472c1f2
# The log-likelihood of the test outputs
logpdf(p_fx(x_test, noise_var), y_test)

# ╔═╡ 2b035209-ec07-4cf9-aea8-56aca90b2ee4
begin
	scatter(
	    x_train,
	    y_train;
...
# ╔═╡ 4f6ef0ad-0f5e-4b75-967e-f24691a3e4d8
begin
	...
	println("The log-likelihood of the train outputs: ", logpdf(fx_1, y_train))
	
	# compute the posterior Gaussian process given the training data
	p_fx_1 = posterior(fx_1, y_train)
	println("The log-likelihood of the test outputs: ", logpdf(p_fx_1(x_test, noise_var), y_test))
end

# ╔═╡ 6a82445f-ae43-4e1e-8444-dcb7c20077a8
begin
	scatter(
	    x_train,
	    y_train;
	...
	scatter!(x_test, y_test; label="Test Data")
	plot!(0:0.001:15, p_fx; label="SqExponentialKernel", ribbon_scale=2)
	plot!(0:0.001:15, p_fx_1; label="PolynomialKernel", ribbon_scale=2)
end

# ╔═╡ f93c08eb-077d-4efc-9bd0-a3de7a562856
begin
	...
	# compute the log-likelihood of the outputs of the training dataset
	println("The log-likelihood of the train outputs: ", logpdf(fx_2, y_train))
	
	# compute the posterior Gaussian process given the training data
	p_fx_2 = posterior(fx_2, y_train)
	println("The log-likelihood of the test outputs: ", logpdf(p_fx_2(x_test, noise_var), y_test))
end

# ╔═╡ 97433b36-7b31-4c81-87cc-55cf029a9bca
begin
	scatter(
	    x_train,
	    y_train;
...
# ╔═╡ 6da6b8fb-873c-4d71-9ab4-5fbbd591ed54
-logpdf(p_fx(x_test, noise_var), y_test)

# ╔═╡ ee2b2789-8884-4b79-9c57-e24dfb67eff8
-logpdf(p_fx_1(x_test, noise_var), y_test)

# ╔═╡ 87e19a93-a1f6-46ec-b28a-8543e8afd2a1
-logpdf(p_fx_2(x_test, noise_var), y_test)

# ╔═╡ 336d2ee5-3f01-4813-8189-e7a86bce8c0a
